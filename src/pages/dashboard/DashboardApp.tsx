...
const DashboardApp = () => {
  const getStatusStyles = (status: Booking["status"]) => {
    switch (status) {
      ...
      default:
        return "bg-slate-100 text-slate-800/30 dark:text-slate-300";
    }
  };
  ...
  const columns: ColumnDef<Booking>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge
          className={`${getStatusStyles(row.original.status)} border-0 px-3 py-1 font-medium`}
        >
          {row.original.status}
        </Badge>
      ),
  ...
  return (
    <>
      <div className="flex flex-wrap gap-4 mt-4 mb-8">
        <div className="flex min-w-[200px] flex-1 flex-col gap-2 rounded-xl p-6 bg-white border border-[#cfdbe7] shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-4  ">
        <div className="lg:col-span-2 bg-white shadow-md rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden px-4">
          <div className="flex justify-between pt-6 border-b border-slate-100 dark:border-slate-800">
            ...
            <button className="text-blue-400 capitalize text-sm">
              view all
            </button>
          </div>
        ...
        <div className="bg-white rounded-xl p-6 text-black flex flex-col justify-between shadow-sm ">
          <div>
            ...
            <div className="space-y-4">
              <RoomProgressBar
                data={[
                  { name: "deluxe suite", color: "blue", value: 82 },
                  { name: "Standard", color: "green", value: 52 },
                  { name: "suite", color: "yellow", value: 72 },
                ]}
              />
            </div>
          </div>
